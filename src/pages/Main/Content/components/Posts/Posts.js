...
import { useLiveSearch } from "../../../../../shared/hooks";
...
export const Posts = ({
    ...
    const { value, setValue, filtredPostsList, filtredLikedPostsList } =
        useLiveSearch(postsList, likedPosts);
    ...
    const blogPosts = (
        isLikedPost ? filtredLikedPostsList : filtredPostsList
    )?.map((item, position) => {
        return (
            <PostItem
                id={item.id}
